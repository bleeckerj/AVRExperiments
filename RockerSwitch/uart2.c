...
//	while(!uartReadyTx[nUart]);
	...
	if(nUart)
	{
	  // modified UDRE to point to UDRE1 to be compatible with atmega324
	  /** while(!(UCSR1A & (1<<UDRE))); **/
	...
	{
	  // modified UDRE to point to UDRE0 to be compatible with atmega324
	  /** while(!(UCSR0A & (1<<UDRE))); **/
...
void uart0PauseSendByte(u08 data)
{
  sbi(PORTA, PA7);
  uartSendByte(0, data);
}
...
// fix these interrupt vector names per iomxx4.h in avr-libc to work with mega324p
