...
void uart0Init(void)
{
	...
	outb(UCSR0B, BV(RXCIE)|BV(TXCIE)|(1<<RXEN)|BV(TXEN));
...
//	while(!uartReadyTx[nUart]);
	...
	if(nUart)
	{
	  while(!(UCSR1A & (1<<UDRE1))); // modified UDRE to point to UDRE1 to be compatible with atmega324
		outb(UDR1, txData);
	}
	else
	{
	  while(!(UCSR0A & (1<<UDRE0))); // modified UDRE to point to UDRE0 to be compatible with atmega324
		outb(UDR0, txData);
	}
...
// fix these interrupt vector names per iomxx4.h in avr-libc
UART_INTERRUPT_HANDLER(SIG_USART_TRANS)      
{
...
UART_INTERRUPT_HANDLER(SIG_USART_RECV)      
{
